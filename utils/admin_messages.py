...
CATEGORY_TYPES = {
    # ===== 系统自动操作 =====
    'auto_extend': '自动延期',           # 考试/培训自动延期
    'auto_assign_exam': '自动分配考试',   # 签到后自动分配考试
    'auto_assign_training': '自动分配培训', # 考试完成后自动分配培训
    
    # ===== 管理员操作 =====
    'admin_reset_sign': '重置签名',       # 管理员重置学员签名
    'admin_reset_exam': '重置考试',       # 管理员重置学员考试
    'admin_unassign': '取消分配',         # 管理员取消培训/考试分配
    'admin_delete': '删除记录',           # 管理员删除记录
    'admin_restore': '恢复记录',          # 管理员恢复记录
    'admin_import': '导入数据',           # 管理员导入数据
    'admin_push': '推送通知',             # 管理员推送通知
    
    # ===== 用户行为 =====
    'user_login': '用户登录',             # 用户登录
    'user_signin': '用户签到',            # 用户培训签到
    'user_submit_exam': '提交考试',       # 用户提交考试
    'user_submit_interview': '提交访谈',  # 用户提交访谈
    
    # ===== 系统警告 =====
    'exam_expired': '考试过期警告',       # 考试即将/已过期
    'training_expired': '培训过期警告',   # 培训即将/已过期
}
...
def log_admin_message(
    ...
    try:
        ...
        data = {
            ...
            "metadata": metadata  # JSON 字段，存储额外信息
        }
        ...
        if result.data:
            logger.info(f"📬 管理员消息已记录: [{category}] {title}")
            return True
        return False
...
def log_training_expired_warning(db, training_id: int, training_name: str, expired_at: str):
    """记录培训过期警告"""
    return log_admin_message(
        level="warning",
...
def get_unread_message_count(db=None) -> int:
